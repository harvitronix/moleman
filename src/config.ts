...
import type { AgentConfig, Config, InputSpec, OutputSpec, WorkflowItem } from "./types.js";

export async function loadConfig(configPath: string): Promise<Config> {
  const raw = await fs.readFile(configPath, "utf8").catch((err: unknown) => {
  ...
  try {
    parsed = (YAML.parse(raw) ?? {}) as Partial<Config>;
  } catch (err) {
    if (err instanceof Error) {
  ...
  const baseAgents = await loadBaseAgents(configPath);
...
export function validateConfig(cfg: Config): void {
  if (Object.keys(cfg.agents).length === 0) {
  ...
  validateWorkflow(cfg, cfg.workflow, seenNames);
}
...
async function loadBaseAgents(configPath: string): Promise<Record<string, AgentConfig>> {
  let dir = configDir(configPath);
  if (!dir) {
    dir = ".";
  }
...
function validateWorkflow(cfg: Config, items: WorkflowItem[], seenNames: Set<string>): void {
  items.forEach((rawItem, idx) => {
    ...
    if (type === "loop") {
      ...
      validateWorkflow(cfg, body, seenNames);
      return;
    }
